...
def setup_logger(rank):
    ...
    if rank == 0:  # 只让主进程写日志
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler("train_ddp1.log", mode="w"),
...
######################################################新增1 在损失项增加标签平滑 #########################################
        # 1. 初始化类别权重（词表大小103，默认权重为1）
        vocab_size = 103  # 与你的sympfn_model的vocab_size保持一致
        class_weights = torch.ones(vocab_size)
        
        # 2. 调低特定token的权重（比如token_id=9，可根据需求修改）
        class_weights[9] = 0.1  # 极大惩罚预测token=9的行为
        
       
        self.ce_loss = nn.CrossEntropyLoss(ignore_index=self.tokenizer.pad_id,weight=class_weights.to(self.device),label_smoothing=0.1)
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                if self.global_rank == 0:
                    ...
                    # ========== 核心修改：收集前3个样本并输出到日志 ==========
                    if sample_count < print_sample_num:
                        # 从logits取argmax得到预测token序列
                        pred_token_ids = torch.argmax(logits, dim=-1)
                        # 确定当前batch要收集的样本数（避免超量）
                        current_print_num = min(print_sample_num - sample_count, curr_batch_size)
                        # 转换为列表（类型一致，纯数值）
                        true_token_list = q_f[:current_print_num].cpu().numpy().tolist()
                        pred_token_list = pred_token_ids[:current_print_num].cpu().numpy().tolist()
                    
                        # 逐样本输出到日志
                        for i in range(current_print_num):
                            sample_count += 1
                            logger.info(f"=== Epoch {epoch_idx} | Val Sample {sample_count} ===")
                            logger.info(f"True Token Sequence: {true_token_list[i]}")
                            logger.info(f"Pred Token Sequence: {pred_token_list[i]}")

                del loss, logits, predictions, targets, q_x, q_y, q_f, ctx_x, ctx_y, ctx_f, d_train_embs
                # 主进程额外清理预测变量
                if self.global_rank == 0 and 'pred_token_ids' in locals():
                    del pred_token_ids, true_token_list, pred_token_list
        torch.cuda.empty_cache()
        return total_loss / len(dataloader)
...
#################################修改 batch_size:16->24,32,48,64,72############################################### 新增4 传入断点路径到 Trainer ##################################################
    trainer = PFNTrainer(
    ...
    trainer.run_training_loop(train_files=[data_filename], epochs_per_file=200, logger=logger)
